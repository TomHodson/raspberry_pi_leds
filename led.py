...
parser.add_argument("--flash", '-f', help ='square wave (Hz)',
                    type=float)
...
def set_led(colour):
    #print "set led to {}".format(colour)
    spi.write("".join(chr(int(i)) for i in colour))
    spi.flush()

with open("/dev/spidev0.0", 'w') as spi:
#if True:
    colour = colours.get(args.colour, args.colour)
    colour = [int(cie1931(float(c))) for c in colour.split(',')]
    set_led(colour)
    while args.flash:
        set_led(off)
